...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const [registerTable, { reload, getForm, getPaginationRef, clearSelectedRowKeys, setTableData }] = useTable(
        {
          title: '催收任务',
          scroll: { y: 600 },
          beforeFetch,
      ...
      watch(
        ...
        async (data: any) => {
          ...
          console.log(data, res.data.data.list, '我是watchData')
        },
      )
      ...
      function handleDetail(record: Recordable) {
        window.open(`https://test.gsrental.cn/newAdmin/#/Order_router/orderDetail/${record.orderId}?uid=${record.uid}&name=${record.nickName}&back=/Collection/CollectionTask`)
      }
