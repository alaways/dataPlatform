    ...
    <BasicTable @register="registerTable">
      <template #toolbar>
        <Button
          ...
          @click="aoaToExcel"
          v-if="hasPermission('riskThreeExport')"
        >
          导出记录
        </Button>
      </template>
...
<script lang="ts">
  ...
  import { exportThreeQueryExcel, getThreeQueryList } from '/@/api/riskControl'
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      async function aoaToExcel() {
        ...
        const res = await exportThreeQueryExcel(params)
        downloadByData(res.data, '三要素查询记录.xlsx')
