  ...
  <PageWrapper title="" contentBackground>
    <div class="m-4">
      ...
      <Spin tip="加载中..." :spinning="loading">
        <RiskControl v-if="detailInfo && !loading" :isRiskEnter="true" :detailInfo="detailInfo" />
...
<script lang="ts">
  ...
  import RiskControl from '/@/views/order/orderDetail/components/riskControlOrange/index.vue'
  import { Spin } from 'ant-design-vue'
  import { getOrangeRiskControl } from '/@/api/riskControl'

  export default defineComponent({
    // 有橙风控
    name: 'RiskQueryOrange',
    components: {
      PageWrapper,
      RiskControl,
      BasicForm,
    ...
    setup() {
      ...
      async function handleSearch(value) {
        detailInfo.value = {}
        loading.value = true
        detailInfo.value = await getOrangeRiskControl({
          ...value,
          resultType: 'req_id',
        })
        loading.value = false
