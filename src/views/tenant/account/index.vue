<template>
  <PageWrapper dense contentFullHeight contentClass="flex">
    <BasicTable :canResize="true" @register="registerTable">
      <template #toolbar>
        <a-button type="primary" @click="handleCreate" v-if="hasPermission('TenantAdd')">
          新增租户
        </a-button>
      </template>
      <template #action="{ record }">
        <TableAction
          :actions="[
            {
              ifShow: hasPermission('TenantUpdate'),
              label: '编辑',
            ...
            {
              ifShow: hasPermission('TenantDel'),
              label: '删除',
              popConfirm: {
                title: '是否确认删除',
                placement: 'left',
...
<script lang="ts">
  ...
  export default defineComponent({
    name: 'TenantList',
    components: { BasicTable, PageWrapper, AccountModal, TableAction, PasswordModal },
    setup() {
