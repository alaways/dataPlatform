    ...
    setup(_, { emit }) {
      ...
      const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
        ...
        if (unref(isUpdate)) {
          setFieldsValue({
            ...
            commBusinessLoanSettleRate:
              Number((data.record?.commBusinessLoanSettleRate * 100).toFixed(0)) || 0,
            ...handlenAmount(
              {
      ...
      async function handleSubmit() {
        try {
          ...
          parmas['commBusinessLoanSettleRate'] = parmas.commBusinessLoanSettleRate / 100
