...
<script lang="tsx">
  ...
  import { getVintageAmountList2, getVintageOrderList2 } from '/@/api/statistics'
    ...
    setup(props) {
      ...
      const [registerTable, { getForm, setColumns, setTableData }] = useTable({
        ...
        handleSearchInfoFn: (data) => {
          if (data.historyAttachment2) historyAttachment2.value = data.historyAttachment2
          if (data.merchantCode) merchantCode.value = data.merchantCode
          if (data.overdueDays) overdueDays.value = data.overdueDays
          if (data.rentType) rentType.value = data.rentType
          if (data.spuType) spuType.value = data.spuType
          getData()
        },
      })
      ...
      async function getData(than = false) {
        ...
        if (props.currentKey == '1') {
          ...
          res = await getVintageAmountList2({
            percentage: '1.0',
            overdueDays: overdueDays.value,
            ...
            renderColContent({ model, field }) {
              return (
                <div style={{ display: 'flex', 'flex-direction': 'row', alignItems: 'center' }}>
                  <div style={{ margin: '0 10px 0 20px', border: 'none' }}>数据对比</div>
                  <DatePicker
                    style={{ width: '150px' }}
                    allowClear
