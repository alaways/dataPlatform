  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const getNdata = async (id) => {
        let nres = await getSysApiCache2({
          id,
        })
        loading.value = true
        timer.value = setInterval(async () => {
          loading.value = true
          nres = await getSysApiCache2({
            id,
          })
          if (nres) {
      ...
      async function getData(than = false) {
        ...
        if (props.currentKey == '1') {
          res = await getVintageOrderList2({
            percentage: '1.0',
            ...
            rentModeFullList:
              typeof rentModeFullList.value == 'string'
                ? rentModeFullList.value
                : rentModeFullList.value?.join(','),
            merchantCode: objStatusList[form.merchantCode],
            merchantTerminalNoList: merchantTerminalNoList.value,
          })
        } else {
          const nres = await getOverdueAmountListAsync2(
            {
              percentage: '1.0',
              overdueDays: overdueDays.value,
      ...
      const init = async () => {
        ...
        const data = await getStoreNoPageList({ pageSize: 999999, limit: 999999 })
        //云享租
        yxzList.value =
          data
                ...
                <div style={{ display: 'flex', 'flex-direction': 'row', alignItems: 'center' }}>
                  ...
                  <DatePicker
                    ...
                    onChange={(e) => {
                      ...
                      //localStorage.setItem('historyAttachment', e)
                    }}
                  />
                  ...
                  <DatePicker
                    ...
                    onChange={(e) => {
                      ...
                      //localStorage.setItem('historyAttachment2', e)
                    }}
                  />
                </div>
              )
