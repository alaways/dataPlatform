...
<script lang="ts" setup name="statisticsSlippage">
  import { getMainCard, getOverDueOrderDetails } from '/@/api/statistics'
  import { onMounted, ref } from 'vue'
  import { summaryList, searchFormSchema } from './data'
  ...
  async function init() {
    loading.value = true
    getOverDueOrderDetails({ spuType: spuType.value, rentType: rentType.value }).then(
      (res: any) => {
        res = res.data[0]
        ...
        getMainCard({ spuType: spuType.value, rentType: rentType.value }).then((req) => {
          summaryData.value.forEach((ch) => {
            const countExpireOrder = Number(req.data['effectOrderCount']) || 0
            const countExpireTotalAmount2 = Number(req.data['effectOrderAmount']) || 0
            if (
              [
            ...
            // 订单逾期率 -- new
            if (ch.key == 'yqRate') {
              const yq = Number(req.data.overDueOrderCount) / countExpireOrder
              ch.num = handleToFixed2(yq * 100)
            }
            // 订单逾期率
            if (ch.key == 'countOverdueAmountrRate') {
              const rate = Number(req.data['overDueOrderCount']) / countExpireOrder
            ...
            if (ch.key == 'sumAmountrRate') {
              const rate = Number(req.data['overdueAmount']) / countExpireTotalAmount2
              ch.num = Number((rate * 100).toFixed(2))
            }
            // 15天逾期率
