<template>
  <Card title="订单汇总">
    <HeaderShow :timer="String(timer)" :isAsync="true" @resetHanlder="resetHanlder" />
    <!-- 表格数据 -->
...
<script lang="ts">
  ...
  import { getOrderTest15 } from '/@/api/statistics/index'
  ...
  export default defineComponent({
    name: 'TotalAllPie',
    components: { HeaderShow, Card, BasicTable, Spin },
    setup() {
      const { hasPermission } = usePermission()
      const [registerTable, { setTableData, getForm, setLoading, getPaginationRef }] = useTable({
        title: '',
        beforeFetch,
      ...
      const resetHanlder = async () => {
        ...
        const nres = await getOrderTest15(payload, { updateForce: 1 })
        id.value = nres?.id
        if (nres?.id) {
      ...
      const getListData = async (payload) => {
        loading.value = true
        const nres = await getOrderTest15(payload, { updateForce: 0 })
        id.value = nres?.id
        setLoading(false)
        if (nres?.id) {
          timer.value = nres?.updateTime
