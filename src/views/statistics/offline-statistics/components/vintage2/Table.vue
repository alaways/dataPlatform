    ...
    setup(props) {
      ...
      async function getData(than = false) {
        ...
        const objStatusList = {
          ...
          ZYFOther: otherJZList.value?.join(','),
        }
      ...
      async function init() {
        const form = await getForm()
        const data = await getStoreList({ pageSize: 999999, limit: 999999 })
        ...
        otherList.value =
          data
            ?.filter((item: any) => item.merchantCode != 'YXZ')
            ?.map((item: any) => item.merchantCode) || []
        otherJZList.value = data?.filter((item: any) => item.merchantCode != 'JZYF')
            ?.map((item: any) => item.merchantCode) || []
