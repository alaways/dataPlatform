<template>
  <div>
    <BasicTable @register="registerTable">
      <template #toolbar>
        <Button type="primary" @click="handleFine" v-if="hasPermission('orderJZbatchChangePay')">
          批量修改罚金
        </Button>
        <Button type="primary" @click="handlePayment" v-if="hasPermission('orderJZbatchPay')">
          批量收款
        </Button>
      </template>
      <template #action="{ record }">
        <TableAction
          ...
          :actions="[
            {
              ifShow: hasPermission('orderJZDetail'),
              label: '详情',
...
<script lang="tsx">
  ...
  import { reducePenaltyAmount, orderBillItemPage } from '/@/api/order'
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      function handlePayment() {
        ...
        const totalAmount = rowList.value.reduce((sum, item) => sum + item.repayAmount, 0)
