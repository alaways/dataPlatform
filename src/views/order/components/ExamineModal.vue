  ...
  >
    <Spin tip="正在加载业务员所属地区..." size="large" :spinning="loading">
      <BasicForm @register="registerForm">
        ...
        <template #workTxt>
          <h2 class="m-0 mr-12 text-base">用户收入来源</h2>
        </template>
      </BasicForm>
    </Spin>
  </BasicModal>
...
<script lang="ts">
  ...
  import { contractType } from '../../contract/seal/data'
    ...
    setup(_, { emit }) {
      ...
      async function handleSubmit() {
        try {
          ...
          if (formValues.lockFeeSwitch) {
            formValues.lockFee = 0
          }

          if (typeof values.salespersonId == 'number') {
            // 选择ID的情况
            console.log(values.salespersonId)
          } else {
            // 输入的情况
            formValues['salesperson'] = formValues.salespersonId
            delete formValues.salespersonId
          }
          if (formValues.workType == '其他') {
            formValues.workType = formValues.workTypeInput
          }
          delete formValues.workTypeInput
