...
import { isAdmin, isString } from '/@/utils/is'
...
const transform: AxiosTransform = {
  ...
  transformResponseHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    if (!options.hideErrorMsg) throw new Error(timeoutMsg || t('sys.api.apiRequestFailed'))
  },
  ...
  responseInterceptorsCatch: (axiosInstance: AxiosResponse, error: any) => {
    ...
    const hideErrorMsg = config?.requestOptions?.hideErrorMsg
    ...
    try {
      ...
      if (errMessage) {
        if (errorMessageMode === 'modal') {
          ...
          if (!hideErrorMsg) createMessage.error(errMessage)
        }
    ...
    deepMerge(
      {
        ...
        requestOptions: {
          ...
          // 接口地址
          apiUrl: isChecked ? getCutHost(_urlPrefix) : globSetting.apiUrl,
          // 接口拼接地址
          urlPrefix: isChecked ? '' : urlPrefix,
          //  是否加入时间戳
          joinTime: true,
          // 忽略重复请求
